...
FROM ALL_TAB_COLUMNS WHERE OWNER = :schemaname AND TABLE_NAME = :tablename";
		...
		public override IEnumerable<DatabaseSchemaInfo> GetSchemas() {
			using (DbConnection connection = GetConnection()) {
				...
				using (DbCommand command = connection.CreateCommand()) {
					...
					using (DbDataReader reader = command.ExecuteReader())
					{
						while (reader.Read())
						{
							yield return new DatabaseSchemaInfo((long)reader.GetInt64(reader.GetOrdinal("USER_ID")), reader.GetString(reader.GetOrdinal("USERNAME")), reader.GetDateTime(reader.GetOrdinal("CREATED")));
						}
					}
				}
			}
		...
		public override DatabaseSchemaInfo GetSchema(string schemaName) {
		...
		public override IEnumerable<DatabaseTableInfo> GetTables(string schemaName) 
		{
			using (DbConnection connection = GetConnection()) {
				connection.Open();
				using (DbCommand command = connection.CreateCommand())
				{
					...
					using (DbDataReader reader = command.ExecuteReader()) {
						while(reader.Read()) {
							DatabaseTableInfo dti = BuildDatabaseTableInfo(reader);
		...
		private IEnumerable<DatabaseColumnInfo> GetDatabaseColumnsInfo(string schemaName, string tableName) 
		{
			using (DbConnection connection = GetConnection()) {
				connection.Open();
				using (DbCommand command = connection.CreateCommand())
		...
		public override DatabaseTableInfo GetTable(string schemaName, string tableName) 
		{
		...
		private DatabaseTableInfo BuildDatabaseTableInfo(DbDataReader reader) 
		{
		...
		private DatabaseColumnInfo BuildDatabaseColumnInfo(DbDataReader reader) 
		{
			DatabaseColumnInfo dci = new DatabaseColumnInfo();
